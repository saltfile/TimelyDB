        ...
        const std::shared_ptr<ExecutionPlanNode> & root)
{
    switch (node->type())
    {
        ...
        {
            auto s = std::dynamic_pointer_cast<Program>(node);
        ...
        {
            auto sys = std::dynamic_pointer_cast<System>(node);
            std::shared_ptr<ExecutionPlanNode> where;
            if (sys->m_where != nullptr)
            {
                auto e = std::dynamic_pointer_cast<Infix>(sys->m_where);  // 先将类型转换为中缀表达式
        ...
        {
            auto show = std::dynamic_pointer_cast<Show>(node);
        ...
        case Node::NODE_USE:  // use 处理
        {
            ...
            auto use = std::dynamic_pointer_cast<Use>(node);
            auto u = eval_use(use);
            root->set_child(u);
            return u;
        }
        ...
        {
            auto create = std::dynamic_pointer_cast<Create>(node);
        ...
        {
            auto select = std::dynamic_pointer_cast<Select>(node);
            return eval_select(select, root);
        }
        case Node::NODE_INSERT:  // insert 处理
        {
            auto insert = std::dynamic_pointer_cast<Insert>(node);
        ...
        {
            auto s = std::dynamic_pointer_cast<String>(node);
            return eval_string(s);
        }
        case Node::NODE_INTEGER:  // integer
        {
            auto s = std::dynamic_pointer_cast<Integer>(node);
            return eval_integer(s);
        }
        case Node::NODE_FLOAT:  // float
        {
            auto s = std::dynamic_pointer_cast<Float>(node);
            return eval_float(s);
        }
        case Node::NODE_INFIX:  // 中缀表达式
        {
            auto s = std::dynamic_pointer_cast<Infix>(node);
        ...
        {
            auto s = std::dynamic_pointer_cast<Prefix>(node);
            return eval_prefix(s, env, root);
        }
        case Node::NODE_NULL:  // null
        {
            auto s = std::dynamic_pointer_cast<Null>(node);
            return eval_null(s);
        }
        case Node::NODE_IDENTIFIER:  // 用户定义的变量
        {
            auto s = std::dynamic_pointer_cast<Identifier>(node);
