...
handler_event load_arr[WHEEL_SIZE];//时间任务准备
extern int wheel_idx = 0;//60格
bool load_flag = false;

ThreadPool handler_pool(4);//处理任务的池子

void* load_fun(void *arg){
    while (1){
        if (load_arr[wheel_idx].is_task){
            handler_pool.enqueue([]{
                load_arr[wheel_idx].f_fun();
            });
...
void* setting_task(void *avg){
    ...
    for (int i = flag; i >= 0; i-=interval) {
        load_arr[i].is_task = true;
        load_arr[i].f_fun = load_disk;
    }
...
void run_file_loading(){
    ...
    pthread_t tid;
    pthread_create(&tid,NULL,load_fun,NULL);
    pthread_t  tid2;
